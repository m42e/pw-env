...
"#;
    ...
    /// Kills the `delete ! in GpgBackend::gpg_file_path` mutant (line 120).
    /// When the target file *exists*, canonicalize() succeeds and the real
    /// starts_with check must trigger the fallback for an escaping path.
    #[test]
    fn gpg_file_path_rejects_canonicalized_path_outside_dir() {
        let parent = tempfile::TempDir::new().unwrap();
        // A real file in the parent dir — canonicalize() will succeed for this path.
        let escape_file = parent.path().join("escape.gpg");
        std::fs::write(&escape_file, "").unwrap();
        // ctx_dir is a subdirectory; "../escape.gpg" resolves to the parent's file.
        let sub_dir = parent.path().join("subdir");
        std::fs::create_dir(&sub_dir).unwrap();

        let config = Config {
            defaults: Defaults {
                gpg: crate::config::GpgConfig {
                    file_pattern: "../escape.gpg".to_string(),
                    recipient: None,
                },
                ..Defaults::default()
            },
        ...
        let path = GpgBackend::gpg_file_path(&sub_dir, &config);
        assert_eq!(path, sub_dir.join(".env.gpg"));
    }

    /// Kills the three body-replacement mutants for `load_all_stored_secrets` (line 152).
    /// Verifies that the actual decrypted content is returned, not an empty or stub map.
    #[test]
    fn load_all_stored_secrets_returns_expected_values() {
        let gpg_content = "KEY1=value1\nKEY2=value2\n";
        with_mock_gpg(gpg_content, || {
            let temp_dir = tempfile::TempDir::new().unwrap();
            let gpg_file = temp_dir.path().join(".env.gpg");
            std::fs::write(&gpg_file, "placeholder").unwrap();
            ...
            let ctx = make_gpg_resolve_context(&config, temp_dir.path());
            let result = GpgBackend::load_all_stored_secrets(&ctx).unwrap();
            assert_eq!(result.len(), 2, "expected exactly 2 entries");
            assert_eq!(result["KEY1"].value, "value1");
            assert_eq!(result["KEY2"].value, "value2");
        });
    }

    /// Kills the `delete ! in GpgBackend::load_all_stored_secrets` mutant (line 153).
    /// When the GPG file does not exist, the function must return an error.
    #[test]
    fn load_all_stored_secrets_returns_err_when_file_missing() {
        let temp_dir = tempfile::TempDir::new().unwrap();
        // No .env.gpg file created here.
        let config = Config {
            defaults: Defaults::default(),
            log: LogConfig::default(),
            updates: UpdateConfig::default(),
        ...
        assert!(result.is_err(), "expected error when GPG file is missing");
    }

    /// Kills the `replace tests::with_mock_gpg with ()` mutant (line 512).
    /// An assertion *outside* the closure proves that `f()` was actually invoked.
    #[test]
    fn with_mock_gpg_executes_closure() {
        let mut called = false;
        with_mock_gpg("", || {
            called = true;
        });
        assert!(called, "with_mock_gpg must invoke the provided closure");
    }
}
